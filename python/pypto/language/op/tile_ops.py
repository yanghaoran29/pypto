...
def load(
    ...
    MX scale GM tensors use ``pl.Tensor[..., pl.MX_A_ZZ]`` / ``pl.MX_B_NN``
    (same shorthand as ``pl.NZ``). Loading them uses the MX Mat path; the load
    still lands in L1/Mat — use ``pl.move`` to LeftScale/RightScale afterwards.
