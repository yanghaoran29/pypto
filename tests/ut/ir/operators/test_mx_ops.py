...
"""Unit tests for MXFP8 matmul path: matmul_mx/acc/bias, tget_scale_addr, mx load."""
...
class TestMatmulMxAccBias:
    def test_matmul_mx_acc_type(self):
        span = ir.Span.unknown()
        ...
        rhs = ir.Var("rhs", ir.TileType([64, 32], DataType.FP8E4M3FN), span)
        rhs_scale = ir.Var("rhs_scale", ir.TileType([2, 32], DataType.FP8E8M0), span)
        call = ir.op.tile.matmul_mx_acc(acc, lhs, lhs_scale, rhs, rhs_scale, span)
        assert call.op.name == "tile.matmul_mx_acc"
        assert isinstance(call.type, ir.TileType)
        assert call.type.dtype == DataType.FP32

    def test_matmul_mx_bias_type(self):
        span = ir.Span.unknown()
        lhs = ir.Var("lhs", ir.TileType([16, 64], DataType.FP8E4M3FN), span)
        lhs_scale = ir.Var("lhs_scale", ir.TileType([16, 2], DataType.FP8E8M0), span)
        rhs = ir.Var("rhs", ir.TileType([64, 32], DataType.FP8E4M3FN), span)
        ...
        call = ir.op.tile.matmul_mx_bias(lhs, lhs_scale, rhs, rhs_scale, bias, span)
        assert call.op.name == "tile.matmul_mx_bias"
        assert isinstance(call.type, ir.TileType)
        assert call.type.dtype == DataType.FP32
    ...
    def test_matmul_mx_acc_spec(self):
        ...
        assert spec is not None
        assert spec["output_memory"] == ir.MemorySpace.Acc
        assert spec["input_constraints"][0] == [ir.MemorySpace.Acc]

    def test_matmul_mx_bias_spec(self):
        spec = ir.get_op_memory_spec("tile.matmul_mx_bias")
        assert spec is not None
        assert spec["input_constraints"][4] == [ir.MemorySpace.Bias]
