...
def _mx_rhs_matmul_inputs() -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
    """Reuse the exact-scale pattern with quantization on the matmul RHS."""
    b_source, selector_kn, lhs_scale = _mx_matmul_inputs()
    lhs = selector_kn.transpose(0, 1).contiguous()
    return lhs, lhs_scale, b_source
...
class MixCase(PTOTestCase):
    ...
    def define_tensors(self) -> list[TensorSpec]:
        ...
        assert scale_boxes[0] >= 2 and scale_boxes[1] >= 2
        a, b, b_scale = _mx_matmul_inputs()
        return [
            TensorSpec("a", [M, K], DataType.FP32, init_value=a),
            TensorSpec("b", [K, N], DataType.FP8E4M3FN, init_value=b),
