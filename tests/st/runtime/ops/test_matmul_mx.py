...
"""a5/a5sim ST for MX DSL: load scales → tget_scale_addr → matmul_mx(/acc/bias).

Minimal host-prequant MXFP8 path:
...
dtype FP8E4M3FN (Ascend950 rejects FP8E5M2 for MX data here).
"""
...
def matmul_mx_acc_prequant(
    ...
    """Accumulate two identical MX matmuls into Acc (expected = 2 * base golden)."""
    with pl.at(level=pl.Level.CORE_GROUP):
        ta = pl.load(a, [0, 0], [128, 64], target_memory=pl.Mem.Mat)
        tas = pl.load(a_s, [0, 0], [128, 2], target_memory=pl.Mem.Mat, mx_layout="mx_a_zz")
        tb = pl.load(b, [0, 0], [64, 64], target_memory=pl.Mem.Mat)
        ...
        rbs = pl.tget_scale_addr(rbs, rb)
        tile_c = pl.matmul_mx(la, las, rb, rbs)
        tile_c = pl.matmul_mx_acc(tile_c, la, las, rb, rbs)
        pl.store(tile_c, [0, 0], c)
    return c


@pl.jit
def matmul_mx_bias_prequant(
    a: pl.Tensor[[128, 64], pl.FP8E4M3FN],
    a_s: pl.Tensor[[128, 2], pl.FP8E8M0],
    b: pl.Tensor[[64, 64], pl.FP8E4M3FN],
    b_s: pl.Tensor[[2, 64], pl.FP8E8M0],
    ...
    with pl.at(level=pl.Level.CORE_GROUP):
        ...
        tb = pl.load(b, [0, 0], [64, 64], target_memory=pl.Mem.Mat)
        tbs = pl.load(b_s, [0, 0], [2, 64], target_memory=pl.Mem.Mat, mx_layout="mx_b_nn")
        tbias = pl.load(bias, [0, 0], [1, 64], target_memory=pl.Mem.Mat)
        la = pl.move(ta, target_memory=pl.Mem.Left)
        las = pl.move(tas, target_memory=pl.Mem.LeftScale)
        rb = pl.move(tb, target_memory=pl.Mem.Right)
        rbs = pl.move(tbs, target_memory=pl.Mem.RightScale)
        bias_l0 = pl.move(tbias, target_memory=pl.Mem.Bias)
        las = pl.tget_scale_addr(las, la)
        rbs = pl.tget_scale_addr(rbs, rb)
        tile_c = pl.matmul_mx_bias(la, las, rb, rbs, bias_l0)
        pl.store(tile_c, [0, 0], c)
    return c


@pytest.mark.platforms("a5", "a5sim")
class TestMatmulMxAccBias:
    """MX matmul_acc / matmul_bias system tests on Ascend950 / a5sim."""

    def test_matmul_mx_acc_compiles_ptoas(self, test_config):
        matmul_mx_acc_prequant._cache.clear()
        assert matmul_mx_acc_prequant.compile(config=test_config) is not None

    def test_matmul_mx_bias_compiles_ptoas(self, test_config):
        matmul_mx_bias_prequant._cache.clear()
        assert matmul_mx_bias_prequant.compile(config=test_config) is not None

    @pytest.mark.platforms("a5")
    def test_matmul_mx_acc_on_device(self, test_config):
        matmul_mx_acc_prequant._cache.clear()
        a, a_s, b, b_s, c, expected = _make_case1_inputs()
        matmul_mx_acc_prequant(a, a_s, b, b_s, c, config=test_config)
        assert torch.allclose(c, expected * 2, rtol=_EPS, atol=_EPS), (
            f"matmul_mx_acc failed: max diff = {(c - expected * 2).abs().max().item()}"
        )

    @pytest.mark.platforms("a5sim")
    def test_matmul_mx_acc_on_a5sim(self, test_config):
        matmul_mx_acc_prequant._cache.clear()
        a, a_s, b, b_s, c, expected = _make_case1_inputs()
        matmul_mx_acc_prequant(a, a_s, b, b_s, c, config=test_config)
        assert torch.allclose(c, expected * 2, rtol=_EPS, atol=_EPS), (
            f"matmul_mx_acc a5sim failed: max diff = {(c - expected * 2).abs().max().item()}"
        )

    @pytest.mark.platforms("a5")
    def test_matmul_mx_bias_on_device(self, test_config):
        matmul_mx_bias_prequant._cache.clear()
        a, a_s, b, b_s, c, expected = _make_case1_inputs()
        bias = torch.full((1, _N), 0.25, dtype=torch.float32)
        matmul_mx_bias_prequant(a, a_s, b, b_s, bias, c, config=test_config)
        assert torch.allclose(c, expected + 0.25, rtol=_EPS, atol=_EPS), (
            f"matmul_mx_bias failed: max diff = {(c - (expected + 0.25)).abs().max().item()}"
        )

    @pytest.mark.platforms("a5sim")
    def test_matmul_mx_bias_on_a5sim(self, test_config):
        matmul_mx_bias_prequant._cache.clear()
        a, a_s, b, b_s, c, expected = _make_case1_inputs()
        bias = torch.full((1, _N), 0.25, dtype=torch.float32)
        matmul_mx_bias_prequant(a, a_s, b, b_s, bias, c, config=test_config)
        assert torch.allclose(c, expected + 0.25, rtol=_EPS, atol=_EPS), (
            f"matmul_mx_bias a5sim failed: max diff = {(c - (expected + 0.25)).abs().max().item()}"
        )
