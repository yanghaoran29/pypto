...
本 PR 落地 **最小 host-prequant MXFP8 matmul** 路径（`tget_scale_addr` + `matmul_mx`）。
...
| `tile.tget_scale_addr` / `pl.tget_scale_addr` | 从 Left/Right 绑定 scale 地址（A5） |
...
| `#pto.layout` / mx load | `mx_a_zz` / `mx_b_nn` / …；本阶段 ST 用 **host ZZ/NN**（AZZ2ZZ） |
| 本阶段覆盖 | `pto.tmatmul.mx` + `pto.tget_scale_addr` |
