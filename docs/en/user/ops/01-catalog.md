...
| Operator | Reach | What it does |
| -------- | ----- | ------------ |
| `quant_mx` | `pl.` (t) | Ascend950 MX block-32 dynamic quantization to FP8E4M3FN (MXFP8) or FP4 (MXFP4 E2M1, logical nibble shape) data plus FP8E8M0 scales (`group_axis` = PTOAS `grpAxis`). MXFP4 accepts FP16/BF16 sources only. May feed `matmul_mx` in one InCore mixed task through direct data+scale V2C transport (see [types](../language/00-types.md)) |
