...
2. **Materialize level3 scratch**: Under the PyPTO or DSA-RP planner, insert ordinary Vec `tile.create` values for missing compiler-owned scratch. `tile.ci` and narrowing `tile.cast` remain A2/A3-only, while required `tile.sort32` scratch is materialized on both A2/A3 and A5. Explicit caller tmp on `tile.sel` / `tile.sels` / `tile.prelu` is preserved. A narrowing `tile.cast` gets scratch **only** when it carries an explicit `saturation_mode=0`: that buffer exists for PTOAS's non-saturating helper, and the saturating form — which is the default, so also the shape a pass-synthesized cast has — is a native conversion that never reads it (see [LegalizeTileCast](17-legalize_tile_cast.md)). The PTOAS planner remains unchanged because its level-2 `PlanMemory` owns implicit scratch.
...
[MemoryReuse](36-memory_reuse.md#declared-allocations) for why an author would want it.
...
[MemoryReuse](36-memory_reuse.md#declared-allocations). Tiles on *different* slots are
...
[`FlattenTileNdTo2D`](14-flatten_tile_nd_to_2d.md#batched-accumulators-pack-along-columns)
