...
Additionally, `tile.load` of a `TensorLayout.MX_A_ZZ` / `MX_B_NN` source resolves to `Mat` and preserves the MX `fractal=32` TileView (does not rebuild via ordinary Mat `GetImplicitTileView`). The load still targets L1/Mat; use a later `tile.move` for Scale.
