...
**When to use**: In the `Default` strategy, after `AllocateMemoryAddr` (so memory addresses are already assigned for any tensors that survive) and before `DeriveCallDirections` and the trailing `Simplify`. It is the last tensor-shape rewrite before call-direction inference.
