...
L0 tiling for static 2D plain and MX `tile.matmul`, `tile.matmul_acc`, and `tile.matmul_bias`: pick an L0 tile shape `(m, n, k)` from the active backend's L0 capacities and rewrite the call into a 2-stage pipelined K-loop with per-iter `tile.extract`s. MX rewrites currently cover K-only tiling: they use 64-element K alignment, keep the full `[M, K/32]` and `[K/32, N]` scale tiles resident in Mat, extract matching K windows directly into `LeftScale` / `RightScale`, and pipeline the data and scale transfers as one operand bundle. Capacity checks use the accumulator's physical L0C footprint, not just its logical shape. When that footprint exceeds L0c, the pass M/N-tiles a fresh plain or biased matmul, or the canonical frontend split-K create/pipeline/store chain, into `[m, n]` output sub-tiles; MX calls that would require M/N tiling remain unchanged.

## Overview

Mat-resident matmuls produced upstream by `ConvertTensorToTileOps` + [`FlattenTileNdTo2D`](14-flatten_tile_nd_to_2d.md) carry full `(M, N, K)` operand shapes — almost always larger than the cube unit's L0a/L0b/L0c capacity. This pass picks an L0-fitting `(m, n, k)` and rewrites the matmul into a K-loop whose body extracts `[m, k]` and `[k, n]` slabs into `Left` / `Right` and accumulates into an `Acc`-resident iter-arg. For MX operations it keeps each single-use direct scale load as one Mat-resident full tile, then extracts the `ko/32` window directly into `LeftScale` / `RightScale` and emits the matching MX accumulate operation. [`BlockMxScaleTensorViews`](16-block_mx_scale_tensor_views.md) physicalizes the retained full scale loads once; the K-loop no longer reconstructs a GM load for every window. MX calls whose scale operands are not direct, single-use loads remain unchanged. The loop is marked `ForKind::Pipeline` with `pipeline_stages=2` so the downstream [`LowerPipelineLoops`](31-lower_pipeline_loops.md) pass produces a 2-deep ping-pong on each complete data-and-scale operand bundle. For example, an FP8 `Right[256, 256]` panel that occupies all 64 KiB of A5 L0B becomes two pipelined `Right[128, 256]` panels, allowing two physical L0B stages without changing the public matmul API.
...
**Pipeline position**: After [`LegalizeTileCast`](17-legalize_tile_cast.md), before [`CanonicalizeTileSlice`](19-canonicalize_tile_slice.md), [`BlockMxScaleTensorViews`](16-block_mx_scale_tensor_views.md), and [`InferTileMemorySpace`](20-infer_tile_memory_space.md). All tile ops and MX scale loads are still logical 2D values, and memory spaces have not yet been inferred.
...
| `tile.matmul_mx`, `tile.matmul_mx_acc`, or `tile.matmul_mx_bias` with single-use direct logical scale loads, output fits L0c, and K requires tiling | Rewritten to a 2-stage pipelined K-loop with matched data and scale windows; MX M/N tiling is deferred |
...
| Output exceeds L0c but no M/N placement applies — including MX, a non-canonical standalone `matmul_acc`, Vec left, a non-matmul-operand consumer, or a chained-matmul scratch whose `[M, N]` exceeds Mat/L1 | Skipped with `PerfHint` (`PH-AT-006`) |
| `K` not a multiple of the operation's required K alignment (16 for plain, 64 for MX) | Skipped with `PerfHint` (`PH-AT-007`) — no aligned K-tiling |
...
| `PH-AT-006` | Output exceeds L0c but no supported M/N placement applies — MX M/N tiling is deferred; for plain `tile.matmul_acc`, this specifically means a caller-owned accumulator outside the canonical create/split-K-pipeline/store chain, or a 4-operand call inside an otherwise canonical chain whose `init_cond` is not a split-K seed test on the loop's induction variable. It also covers a Vec left operand, an already-Bias-resident `tile.matmul_bias` source that would need N sub-windowing, a result consumed on-chip that is not entirely a matmul operand, or a chained-matmul scratch that exceeds Mat/L1 capacity. The canonical split-K case from issue #2232 does not emit this hint. |
| `PH-AT-007` | `K` violates the operation's required alignment (16 for plain, 64 for MX), so no aligned K-tiling exists and the matmul is left untouched |
