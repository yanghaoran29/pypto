...
The lowering creates source-dtype `max` / `scaling` write-only workspace tiles, then binds a value-returning `tile.tquant_mx_raw(src, max, scaling)` whose `TupleType{INT8 dst, UINT8 exp}` (MXFP8) or `{FP4 dst, UINT8 exp}` (MXFP4) elements are projected with `TupleGetItem`. Codegen resolves those projections via `ResolveTupleResultElements` and emits `pto.tquant.mx` with four outs (dst, exp, max, scaling) and `quant_type` `MXFP8` or `MXFP4_E2M1`. Public `group_axis` matches PTOAS `grpAxis`: axis1 keeps `[M,K]` and returns scale `[M,K/32]`; axis0 first transposes `[N,K]` to `[K,N]` and returns scale `[K/32,N]`. Both forms then bind value-returning `tile.tmov_x2zz(exp, tmp)`. Axis1 tmp capacity is `64 + ceil(rows/16)*cols` bytes (33-byte-aligned); axis0 uses a minimal 32-byte Vec pad required by `TMovDnTo2Zz`. Axis0 follows pto-isa `TMovDnTo2Zz` (pin `be5ccb76`): DN `[M̂,N]` → ZZ `[N,M̂]` row/row, then a zero-copy `tile.transpose_view` yields the public `[M̂,N]` col/col scale. MXFP8 adds a zero-copy FP8 data alias and FP8E8M0 scale alias; MXFP4 data is already FP4, so only the scale is aliased. Write effects on the workspace args keep the Calls alive under DCE when a public result is unused.
