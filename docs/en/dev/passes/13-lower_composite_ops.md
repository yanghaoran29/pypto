...
**Src densify (before `tquant_mx_raw`):** PTOAS TQUANT assumes a static ND Vec (`row_major` / `none_box`) that does not alias the outs. Zero-copy views can make a vector-chain result look statically valid without materializing a fresh buffer. Conservatively, if SSA cannot prove the (post-axis0-transpose) src is `tile.load` through buffer-aliasing views / `reinterpret_view`, the rule binds `tile.move(src, Mem.Vec, row_major, none_box)` as `tq_src_dense` and feeds that to `tquant_mx_raw`. A direct `quant_mx(load)` on axis1 therefore skips the extra move; gate-style `transpose → col_expand_mul → … → quant_mx` always densifies. Axis0's inserted transpose is treated as non-load, so a further densify move may follow (correctness over eliding the copy).
