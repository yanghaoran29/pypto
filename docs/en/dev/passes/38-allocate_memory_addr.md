...
A symbolic relative offset is never dropped. Such an offset appears when a view's slice offset is a scalar Var — a constant bound inside an `if`, loop, or `pl.spmd` body, where Simplify does not substitute it, or a genuinely runtime row — as well as for a declared allocation's runtime slot index. Collapsing it onto `slot_base` would make a reshape of that slice read the parent's first rows. The pass first folds every scalar Var assigned exactly once to a constant into the offset, so a constant row yields a constant address; this is required, because the final Simplify later substitutes such a constant into the statements and deletes its binding without remapping MemRef offsets. An offset that still names a runtime value becomes the expression `slot_base + member.byte_offset`, which codegen lowers into the tile's runtime address assignment. Both planners (`PYPTO` and `DSA_RP`) build the address through `MakeAbsoluteMemRefAddress`.
