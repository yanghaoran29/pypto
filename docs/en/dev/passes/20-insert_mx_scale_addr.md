...
4. If a same-block Mat→Scale `tmov` still precedes its paired data tile definition, move that fill after the data def so PTOAS `PTOA5NormalizeTMovPass` bind-before-fill reordering preserves SSA dominance.
