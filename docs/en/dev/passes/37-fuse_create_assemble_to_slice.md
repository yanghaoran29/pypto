...
**When to use**: 37th pass in the `Default` strategy, after `FoldNoOpReshape` and before `DeriveCallDirections` and the trailing `Simplify`. It is the last tensor-shape rewrite before call-direction inference.
