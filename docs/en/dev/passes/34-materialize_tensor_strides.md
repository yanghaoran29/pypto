...
**Position in the default pipeline** (active since RFC #1300 P6): between [`CanonicalizeIOOrder`](33-canonicalize_io_order.md) and [`InitMemRef`](35-init_memref.md). This is the codegen-prep boundary — every layout-mutating pass (`ResolveBackendOpLayouts`, `ExpandMixedKernel`, `SplitVectorKernel`) has finished, and `InitMemRef` is the first consumer that needs explicit stride.
...
- [`CanonicalizeIOOrder`](33-canonicalize_io_order.md) — runs immediately before; produces the program state the materialization consumes
- [`InitMemRef`](35-init_memref.md) — first downstream consumer that depends on explicit stride
