...
This change lands the **host-prequant MXFP8 matmul** path (`tget_scale_addr` +
...
| `tile.matmul_mx_acc` / `pl.matmul_mx_acc` | `Acc, Left, LeftScale, Right, RightScale → Acc` (in-place; `set_output_reuses_input(0)`); same data/scale/alignment checks as `matmul_mx` |
| `tile.matmul_mx_bias` / `pl.matmul_mx_bias` | `Left, LeftScale, Right, RightScale, Bias → Acc`; bias `[1, N]` FP32; same MX checks as `matmul_mx` |
| `tile.tget_scale_addr` / `pl.tget_scale_addr` | Bind scale address from Left/Right (A5); DPS in-place on dst_scale |
...
| Coverage here | `pto.tmatmul.mx` / `.acc` / `.bias` + `pto.tget_scale_addr` |
