...
| `tile.move(..., target_memory=LeftScale/RightScale)` | Mat→Scale move; emits the fill `tmov` in source order (PTOAS may later reorder bind-before-fill relative to `tget_scale_addr`) |
