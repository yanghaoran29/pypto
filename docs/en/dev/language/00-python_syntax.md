...
different one: [`LowerPipelineToSlots`](../passes/29-lower_pipeline_to_slots.md) synthesizes
...
See [InitMemRef](../passes/33-init_memref.md#declared-allocations) and
[MemoryReuse](../passes/35-memory_reuse.md#declared-allocations).
...
| `pl.scope()` | `Runtime(manual=false)` | Orchestrator AUTO scope (`PTO2_SCOPE()`). Hand-placing one requires `@pl.function(auto_scope=False)` (in the default `auto_scope=True` the compiler owns AUTO placement). See [MaterializeRuntimeScopes](../passes/46-materialize_runtime_scopes.md) |
...
> — see [ExpandMixedKernel](../passes/23-expand_mixed_kernel.md#overriding-the-slot-count-slot_num).
