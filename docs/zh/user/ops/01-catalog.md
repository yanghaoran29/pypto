...
| `cast` | `pl.` | 转换 dtype —— 可能展开成多跳链，见 [LegalizeTileCast](../../dev/passes/16-legalize_tile_cast.md) |
| `dim` | `pl.` | 张量的运行期维度 |
| `read` `write` | `pl.` | 元素访问 |
...
push 与 pop 必须**配对**，且每次 pop 都必须有对应的 `tfree`。涵盖这部分的教程尚未编写；机制见 [TPUSH/TPOP](../../reference/pto-isa/01-tpush_tpop.md) 与 [ExpandMixedKernel](../../dev/passes/23-expand_mixed_kernel.md)。
