...
| 算子 | 可达 | 作用 |
| ---- | ---- | ---- |
| `quant_mx` | `pl.` (t) | Ascend950 MX block-32 动态量化，生成 FP8E4M3FN（MXFP8）或 FP4（MXFP4 E2M1，逻辑 nibble shape）数据及 FP8E8M0 scale（`group_axis` 对齐 PTOAS `grpAxis`）。MXFP4 仅接受 FP16/BF16 源。可在同一 InCore mixed task 内通过 data+scale 的直接 V2C 传输供 `matmul_mx` 使用（见 [类型](../language/00-types.md)） |
