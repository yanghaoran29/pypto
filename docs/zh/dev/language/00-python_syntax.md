...
[`LowerPipelineToSlots`](../passes/29-lower_pipeline_to_slots.md) 会为合格 `pl.pipeline` 循环体中
...
参见 [InitMemRef](../passes/33-init_memref.md#声明式分配) 与
[MemoryReuse](../passes/35-memory_reuse.md#声明式分配)。
...
| `pl.scope()` | `Runtime(manual=false)` | orchestrator 的 AUTO scope（`PTO2_SCOPE()`）。手写它需要 `@pl.function(auto_scope=False)`（默认 `auto_scope=True` 下由编译器决定 AUTO 放置）。见 [MaterializeRuntimeScopes](../passes/46-materialize_runtime_scopes.md) |
...
> [ExpandMixedKernel](../passes/23-expand_mixed_kernel.md#覆盖槽位数slot_num)。
