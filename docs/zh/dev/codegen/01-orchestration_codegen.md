...
例如，返回值到参数的追踪（将被调用者返回值映射回 `Out` 参数）是分析工作，应由代码生成之前的 Pass 解决。[`NormalizeReturnOrder`](../passes/27-normalize_return_order.md) pass 现在会在代码生成之前完成此规范化，使编排代码生成可以直接将 `return[i]` 映射到 `out_indices[i]`，无需追踪 `tile.store`/yield 链。
...
（[`NormalizeReturnOrder`](../passes/27-normalize_return_order.md)）保证：
...
[`AutoDeriveTaskDependencies`](../passes/40-auto_derive_task_dependencies.md)，
...
[`ExpandManualPhaseFence`](../passes/41-expand_manual_phase_fence.md) 给自己合成的
