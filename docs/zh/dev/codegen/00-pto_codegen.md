...
| A2/A3 上的 `system.syncall(mode="soft", core_type=C, gm_workspace=ws, used_cores=N)` | `pto.syncall(%gm_ptr[, %used] : !pto.ptr<i32>[, i32]) mode = #pto.sync_all_mode<soft>, core_type = #pto.sync_core_type<C>` | 裸指针 soft barrier；支持全部 participant set；GM workspace 至少 64 字节 |
| A5 上的同一操作（`C=AIV` 或 `MIX`） | `pto.syncall(%gm_view, %ub[, %l1][, %used] : ...) mode = #pto.sync_all_mode<soft>, core_type = #pto.sync_core_type<C>` | partition-view ABI，UB workspace 由编译器插入，MIX 另带 L1 workspace；显式 participant 每个需要 8 个 GM `INT32` 槽位（总数最少 16）；不支持 AIC-only soft |
