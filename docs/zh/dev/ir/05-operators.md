...
MX tensor subview 仍有限制：`tensor.slice`、`tensor.reshape`、`tensor.transpose`、`tensor.reinterpret_view`、`tensor.gather_row`、`tile.gather_row` 与 `pld.tile.remote_load` 都拒绝 MX-layout source。`tensor.view` 唯一的例外是完整 `MX_A_ZZ`/`MX_B_NN` buffer 与其 packed ND backing storage 之间的 shaped、zero-copy FP8E8M0 alias。

FP4 Tensor/Tile shape 与 `valid_shape` 使用逻辑 nibble 数；packed extent 必须为正偶数，slice 起点也不能落在一个字节的第二个 nibble。Runtime 使用物理 x2 carrier，因此 codegen 会在 allocation 与 `tile.set_validshape` 上把 packed axis 减半，同时保持 IR 语义为逻辑 shape。

Grouped-TQUANT、X-to-ZZ、scale memory 与 PTOAS 的详细约束随实现记录在 [LowerCompositeOps](../passes/12-lower_composite_ops.md) 和 [InsertMxScaleAddr](../passes/19-insert_mx_scale_addr.md) 中。
