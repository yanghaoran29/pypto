...
| **一元** | `tile.sqrt` | 逐元素平方根 |
| **量化** | `tile.tquant_mx` / `pl.quant_mx` | 仅 Ascend950 支持的 MX block-32 动态量化，返回 `{FP8E4M3FN\|FP4 quant, FP8E8M0 scale}`；`dtype` 为 `FP8E4M3FN`（MXFP8）或 `FP4`（MXFP4 E2M1，逻辑 nibble shape）。`group_axis` 对齐 PTOAS `grpAxis`（`1` = A 侧 `[M,K]`，`0` = B 侧 `[N,K]` 并转置）。公开 scale shape 为 `[M,K/32]` / `[K/32,N]`；要求完整有效区域和 `K % 64 == 0`（axis1 还要求 `M % 16 == 0`，axis0 还要求 `N % 32 == 0`）。MXFP4 源仅 FP16/BF16。[Pass 13](../passes/13-lower_composite_ops.md) 生成分组 TQUANT 和 X-to-ZZ TMOV。在 mixed task 内，结果可直接经 V2C 供 `matmul_mx` 使用。 |
