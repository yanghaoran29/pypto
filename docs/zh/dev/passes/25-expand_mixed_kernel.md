...
| 形态 | 上报者 |
| ---- | ------ |
| `pl.split_aiv` 区域 | [`AivSplitValid`](99-verifier.md) 检查 (l)，以 [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md) 兜底 |
...
`pl.SplitMode.NONE`——会被 [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md)（pass 23）标记 `split_aiv`，因此
[`SplitVectorKernel`](27-split_vector_kernel.md) 会将其走 split 路径（两个 AIV lane 经由 `dual_aiv_dispatch` 派发），
...
[`LowerAutoVectorSplit`](24-lower_auto_vector_split.md)。
...
[LowerAutoVectorSplit](24-lower_auto_vector_split.md) 对 ragged 边界做了均分时，则取算子上
