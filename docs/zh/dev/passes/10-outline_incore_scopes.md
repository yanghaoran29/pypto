...
[`ConvertTensorToTileOps`](12-convert_tensor_to_tile_ops.md) 把它变成每条 `tile.load`
上的 `cache` kwarg 并擦除它为止。参数索引仅在该窗口内有效 —— 后续 pass 既会向参数列表
追加（[`InjectGMPipeBuffer`](26-inject_gm_pipe_buffer.md)、
...
`SplitAivScopeStmt::split_`，由 [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md)
...
[`LowerAutoVectorSplit`](24-lower_auto_vector_split.md) 下降函数体，并以 `AivSplitLoweredValid` 接替该属性为止。
