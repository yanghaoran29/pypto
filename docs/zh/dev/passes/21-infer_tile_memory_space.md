...
这段夹逼逻辑放在 `StagingSpaceForLoad`（`src/ir/memref.cpp`）里而不是内联在此处，因为本 pass 并非唯一调用者：[`ConvertTensorToTileOps`](12-convert_tensor_to_tile_ops.md#桥接-load-落在哪个空间) 中的 `input_reqs` 桥接，会用同一个函数、依据本阶段读取的同一份 `set_input_memory` 声明，推导它所*创建*的每个 load 的空间。只保留一份，才能保证桥接产生的 load 与本 pass 重定向的 load 不会把同一个操作数放进不同的 buffer。
