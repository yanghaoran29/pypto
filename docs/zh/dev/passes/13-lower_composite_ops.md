...
**Src 稠密化（在 `tquant_mx_raw` 之前）：** PTOAS TQUANT 假定静态 ND Vec（`row_major` / `none_box`）且与 outs 无别名。零拷贝 view 可能让 vector 链产物看起来已有静态 valid，却并未 materialize 新缓冲区。保守策略：若 SSA 无法证明（axis0 转置之后的）src 经 buffer-aliasing view / `reinterpret_view` 追溯到 `tile.load`，则 `Bind` `tile.move(src, Mem.Vec, row_major, none_box)` 为 `tq_src_dense` 再喂给 `tquant_mx_raw`。因此 axis1 上的 `quant_mx(load)` 不插额外 move；gate 类 `transpose → col_expand_mul → … → quant_mx` 一律 densify。Axis0 本规则插入的 transpose 仍视为非 load，其后可再插 densify move（首版偏正确性，不抠掉这次拷贝）。
