...
[`OutlineIncoreScopes`](10-outline_incore_scopes.md) 把这些声明留在 InCore 函数的
...
[`InferTileMemorySpace`](21-infer_tile_memory_space.md) 而言，操作数的 memory space
只有 `OpRegistry` 这一处声明。
...
[`FlattenTileNdTo2D`](15-flatten_tile_nd_to_2d.md) 会把 batch 版展开成 2D 版，同一个
...
[`FlattenTileNdTo2D`](15-flatten_tile_nd_to_2d.md) 对 leading-dim 为 1 的 batch 会逐页
...
[`AutoTileMatmulL0`](19-auto_tile_matmul_l0.md) 在边界处保持合法的原因：该 pass 选择
...
- **内存空间在此显式声明，而不是留给 [`InferTileMemorySpace`](21-infer_tile_memory_space.md)。**
...
- 右操作数被加载进 `Mat`，随后由
  [`InferTileMemorySpace`](21-infer_tile_memory_space.md) 提升到 `Right`，而
...
| rank >= 3 的操作数 | 否 | 它下沉为 `tile.batch_matmul`，其行被 [`FlattenTileNdTo2D`](15-flatten_tile_nd_to_2d.md) 打包成单个 `[B*M, N]` tile —— 分形规则约束的是打包后的尺寸，而非该维度 |
...
本 pass 将两者**各自 1:1**下降为对应的 tile 算子（`tensor.aiv_shard` → `tile.aiv_shard`,`tensor.aic_gather` → `tile.aic_gather`）；此后 IR 与 AUTO `pl.split` 路径经 [`LowerAutoVectorSplit`](24-lower_auto_vector_split.md)（pass 23）产出的结果逐字节一致。随后 `ExpandMixedKernel`（pass 24）将两者折叠进跨核 `tpush`/`tpop` 机制。
