...
TypePtr DeduceTileMoveType(const std::vector<ExprPtr>& args,
  ...
  // Preserve that metadata for the explicit Vec layout conversion and the
  // Vec -> Mat staging layouts accepted by LeftScale/RightScale; ordinary
  // moves must continue to use the destination buffer's boxing granularity.
  const bool is_mx_scale_transport =
      (space == MemorySpace::Vec || space == MemorySpace::Mat) &&
