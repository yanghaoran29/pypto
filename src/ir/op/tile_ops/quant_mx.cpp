...
TypePtr DeduceTileTQuantMxType(const std::vector<ExprPtr>& args,
  ...
  auto groups_dim = std::make_shared<ConstInt>(groups, DataType::INDEX, Span::unknown());
...
TypePtr DeduceTileTMovX2ZzType(const std::vector<ExprPtr>& args,
  ...
  if (group_axis == 1) {
    ...
    const int64_t padded_rows = (dst_rows->value_ + 15) / 16 * 16;
    ...
    const int64_t tmp_bytes = 64 + ((dst_rows->value_ + 15) / 16) * dst_cols->value_;
