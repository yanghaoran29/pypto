...
/// PTOAS materializes these before PlanMemory at level2. PyPTO and DSA-RP emit
/// fixed addresses at level3, so the tmp must become an ordinary tile before
/// this pass initializes MemRefs.
 ...
 private:
  static std::optional<PtoScratchSpec> GetScratchSpec(const CallPtr& call, const Span& span) {
    if (IsOp(call, "tile.ci") && call->args_.size() == 2) {
      ...
      const int64_t cols = result_type->dtype_.GetBit() == 32 ? 192 : 448;
...
FunctionPtr TransformInitMemRef(const FunctionPtr& func) {
  ...
  // and DSA-RP instead emit fixed addresses and invoke level3, so their A2/A3
  // scratch tiles must enter the ordinary MemRef pipeline here. This is the last
  // tile-level boundary: cast legalization and shape/layout rewriting are done,
  // while every synthesized tile can still participate in reuse and placement.
  const MemoryPlanner planner = ctx ? ctx->GetMemoryPlanner() : MemoryPlanner::PyPTO;
  if (handler != nullptr && handler->GetPtoTargetArch() == "a2a3" &&
