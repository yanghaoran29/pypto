...
static std::string MakeTileLoadCodegenPTO(const CallPtr& op, codegen::CodegenBase& codegen_base) {
  ...
  // from the source TensorView (same physical ND strides) so EmitC yields
  // Layout::MX_* for TLoadMxCube*.
  std::string pto_layout;
  if (tensor_type->tensor_view_.has_value()) {
    switch (tensor_type->tensor_view_->layout) {
  ...
  if (is_mx_load) {
    INTERNAL_CHECK_SPAN(tensor_type->shape_.size() == 2, op->span_)
        << "tile.load of MX-layout tensor requires a 2D tensor, got rank " << tensor_type->shape_.size();
